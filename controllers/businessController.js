...
const register = async (req , res)=>{
    try{
        const { businessName, state , city , pincode , businessType , contactEmail , contactPhone , openingHours ,createdAt , businessLogo} = req.body;
        for(const key in req.body)
        {
            if(!req.body[i] || req.body[key].trim() === '')
                {
        ...
        const user = await Business.create({businessName, state , city , pincode , businessType , contactEmail , contactPhone , openingHours ,createdAt , businessLogo})
        console.log(user);
        res.json({
